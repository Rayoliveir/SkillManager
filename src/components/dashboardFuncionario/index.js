...
import { Building, Users, Star, FileText, Edit, Trash2, PenTool, Rocket, Briefcase, CheckCircle, AlertTriangle } from 'lucide-react'; 
...
import './styles.css';
import { gerarHtmlRelatorio, gerarHtmlRelatorioHistorico } from '../../utils/reportTemplate'; // Import corrigido
                                    ...
                                    <div key={av.id} className="evaluation-card">
                                        <div className="evaluation-header">
                                            <div className="evaluation-author">
                                                ...
                                                <div className="evaluation-meta"><span className="evaluation-date">{formatarDataBrasileira(av.dataAvaliacao)}</span><div className="evaluation-rating" style={{color: '#ed8936'}}>{"★".repeat(av.notaDesempenho)}</div></div>
                                            </div>
                                            <div style={{display:'flex', gap:'8px'}}>
                                                <button onClick={() => aoEditarAvaliacao(av)} className="btn-secondary" style={{padding:'5px', minWidth:'auto'}} title="Editar"><Edit size={16} /></button>
...
// --- ABA RELATÓRIOS CORRIGIDA (AGORA COM BOTÃO HISTÓRICO) ---
const AbaRelatorios = ({ estagiarios }) => {
    ...
    const [estagiarioCompleto, setEstagiarioCompleto] = useState(null); // Estado para o histórico
    const [carregando, setCarregando] = useState(false);
    ...
    const handleMudarEstagiario = async (e) => {
        ...
        try {
            ...
            setEstagiarioCompleto(dadosEstagiario); // Guarda para o relatório histórico
            ...
            <div className="info-card">
                ...
                <div className="card-body">
                    <div className="form-group" style={{display: 'flex', gap: '10px', alignItems: 'flex-end'}}>
                        ...
                        {/* BOTÃO QUE TINHA SUMIDO - AGORA ESTÁ AQUI */}
                        {idSelecionado && (
        ...
        <div className="evaluation-form-overlay">
            ...
            <div className="evaluation-form-modal">
                ...
                <form onSubmit={handleSubmit} className="form-body">
                    {/* Seção Pessoal */}
                    <h4 style={{color: '#2C5282', borderBottom: '1px solid #e2e8f0', paddingBottom: '8px', marginBottom: '15px'}}>Dados Pessoais</h4>
                    
                    <div className="form-group">
                        <label>Nome Completo</label>
                        <input className="form-input" name="nome" value={formData.nome} onChange={e=>setFormData({...formData,nome:e.target.value})} required />
                    </div>

                    <div className="form-row">
                        <div className="form-group" style={{flex: 1}}>
                            <label>Email Corporativo</label>
                            <input className="form-input" name="email" value={formData.email} onChange={e=>setFormData({...formData,email:e.target.value})} required />
                        </div>
                        <div className="form-group" style={{flex: 1}}>
                            <label>Cargo</label>
                            <select name="cargo" value={formData.cargo} onChange={e=>setFormData({...formData,cargo:e.target.value})} className="form-input">
                    ...
                    {/* Seção Segurança */}
                    <h4 style={{color: '#2C5282', borderBottom: '1px solid #e2e8f0', paddingBottom: '8px', marginBottom: '15px', marginTop: '20px'}}>Segurança</h4>
                    
                    <div className="form-group">
                        <label>Nova Senha <small style={{color:'#718096', fontWeight:'normal'}}>(Deixe em branco para manter a atual)</small></label>
                        <input className="form-input" name="senha" type="password" value={formData.senha} onChange={e=>setFormData({...formData,senha:e.target.value})} placeholder="••••••••" />
                    </div>
...
function DashboardFuncionario({ initialTab = 'informacoes' }) {
    ...
    const abrirModalEdicaoAvaliacao = (avaliacao) => {
        setEstagiarioParaAvaliar({ id: avaliacao.estagiarioId || avaliacao.estagiario?.id }); 
    ...
    const handleExcluirAvaliacao = async (idAvaliacao, idEstagiario) => {
        try {
            ...
            // Gambiarra necessária para atualizar a lista sem recarregar tudo (que poderia resetar a aba)
            // O ideal seria que carregarDados mantivesse a aba, o que já fazemos via URL.
            carregarDados();
        } catch (error) {
        ...
        try { 
            if (avaliacaoEmEdicao) {
                await api.atualizarAvaliacao(avaliacaoEmEdicao.id, {
                    ...dados,
                    estagiarioId: avaliacaoEmEdicao.estagiarioId || avaliacaoEmEdicao.estagiario?.id
                });
