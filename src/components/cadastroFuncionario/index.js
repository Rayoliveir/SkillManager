...
function CadastroFuncionario() {
    ...
    const handleChange = (e) => {
        ...
        // Aplica Máscaras baseado no nome do campo
        if (name === 'empresaCnpj' || name === 'empresa.cnpj') value = maskCNPJ(value);
        if (keys.includes('cep')) value = maskCEP(value);

        // Atualiza o estado (suporta até 3 níveis de aninhamento)
        if (keys.length === 3) {
            setFormData(prev => ({
                ...prev,
                [keys[0]]: {
                ...
                <fieldset className="empresa-cadastrada">
                    ...
                    {formData.empresaCadastrada === 'sim' ? (
                        <label>CNPJ da empresa
                            <input type="text" name="empresaCnpj" placeholder="00.000.000/0000-00" required value={formData.empresaCnpj} onChange={handleChange} maxLength="18" />
                        </label>
                    ) : (
                        <>
                            <h3>Dados da empresa</h3>
                            <label>CNPJ <input type="text" name="empresa.cnpj" placeholder="00.000.000/0000-00" required value={formData.empresa.cnpj} onChange={handleChange} maxLength="18" /></label>
                            ...
                            <div style={{display: 'flex', gap: '10px'}}>
                                <label style={{flex: 1}}>CEP <input type="text" name="empresa.endereco.cep" placeholder="00000-000" required value={formData.empresa.endereco.cep} onChange={handleChange} maxLength="9" /></label>
