...
public class EmpresaService {
    ...
    public EmpresaResponseDTO salvar(EmpresaDTO dto) {
        ...
        empresa.setCodigoEmpresa(generateUniqueCodigoEmpresa());
    ...
    // --- NOVO MÉTODO: Busca pelo Código Único ---
    public Empresa buscarPorCodigo(String codigo) {
        return empresaRepository.findByCodigoEmpresa(codigo)
                .orElseThrow(() -> new EntityNotFoundException("Empresa não encontrada com o código: " + codigo));
    }
    // --------------------------------------------
    ...
    private String generateUniqueCodigoEmpresa() {
        String codigo;
        do {
            String uuid = UUID.randomUUID().toString().toUpperCase().replaceAll("-", "");
            codigo = uuid.substring(0, 3) + "-" + uuid.substring(3, 6);
        } while (empresaRepository.findByCodigoEmpresa(codigo).isPresent());
        return codigo;
    }
