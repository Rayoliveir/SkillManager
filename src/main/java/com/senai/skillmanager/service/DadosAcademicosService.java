...
public class DadosAcademicosService {
    ...
    public DadosAcademicosResponseDTO salvar(DadosAcademicosDTO dto) {
        ...
        if (dto.getPrevisaoFormatura() != null) {
    ...
    private DadosAcademicosResponseDTO toResponseDTO(DadosAcademicos dados) {
        ...
        try {
            String cleanStr = dados.getPeriodoSemestre().replaceAll("\\D+", "");
            dto.setPeriodoSemestre(cleanStr.isEmpty() ? 0 : Integer.parseInt(cleanStr));
        } catch (Exception e) {
            dto.setPeriodoSemestre(0);
        }

        if (dados.getPrevisaoFormatura() != null) {
